...
@endcan
        ...
        <div class="premium-grid bg-white rounded-3 shadow-sm border">
            ...
            <div class="grid-body">
                @forelse ($data as $key => $user)
                    <div class="grid-row d-grid align-items-center py-3 border-bottom animate__animated animate__fadeInUp"
                        ...
                        <div class="grid-col ps-4">
                            <div class="d-flex align-items-center gap-3">
                                <div class="position-relative d-inline-block">
                                    ...
                                    {{-- Status Dot (Overlapping the bottom right) --}}
                                    <span
                                        class="position-absolute bottom-0 end-0 p-1 border border-2 border-white rounded-circle {{ $user->status ? 'bg-success' : 'bg-secondary' }}"
                        ...
                        <div class="grid-col">
                            <div class="role-tags d-flex flex-wrap gap-1">
                                @forelse($user->getRoleNames() as $role)
                                    <span
                                        class="role-tag role-{{ strtolower(str_replace(' ', '-', $role)) }}">{{ $role }}</span>
                                @empty
                        ...
                        <div class="grid-col">
                            <div class="d-flex flex-column">
                                <span class="text-dark small fw-semibold"><i class="fa-solid fa-phone me-1 text-muted"></i>
                                    {{ $user->phone ?? 'N/A' }}</span>
                                <span class="text-muted small text-truncate" style="max-width: 180px;"><i
                                        class="fa-solid fa-location-dot me-1"></i> {{ $user->address ?? 'Not set' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="grid-col pe-4 text-end">
                            <div class="row-actions d-flex justify-content-end gap-2">
                                ...
                                @can('view', $user)
                                    <a href="{{ route('users.show', $user->id) }}" class="btn btn-sm btn-light border"
                                        title="View Details">
                                ...
                                @can('update', $user)
                                    <a href="{{ route('users.edit', $user->id) }}" class="btn btn-sm btn-light border"
                                        title="Edit Member">
                                ...
                                @can('delete', $user)
                                    <button onclick="handleDelete('{{ $user->id }}')"
                                        class="btn btn-sm btn-light border text-danger" title="Remove Member">
                ...
                @empty
                    <div class="p-5 text-center">
                        <img src="https://cdn-icons-png.flaticon.com/512/6134/6134065.png" style="width: 80px; opacity: 0.5"
                            alt="No results">
        ...
        <div class="grid-footer d-flex justify-content-between align-items-center mt-4">
            <p class="text-muted small mb-0">Displaying {{ $data->firstItem() ?? 0 }}-{{ $data->lastItem() ?? 0 }} of
                {{ $data->total() }} members</p>
